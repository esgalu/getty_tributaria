...
def count_days(df):
    ...
    tmp = df.loc[list_index].Fecha.tolist()

    tmp.insert(0, df.Fecha.loc[0])
    tmp.insert(len(tmp), pd.to_datetime(datetime.today()))

    values = [(tmp[i + 1] - tmp[i]).days for i in range(len(tmp) - 1)]

    list_in_out.insert(0, df['In/Out'].loc[0])
